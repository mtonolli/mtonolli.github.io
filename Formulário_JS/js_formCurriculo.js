...
function validate(item) {
    ...
    if (item == telc) {
        let numTELC = telc.value.replace(/[^0-11]/g, ""); //remove caracteres não-numéricos
        if (validateCPF(numTELC)) {
            item.setCustomValidity(""); //aprovado
        } else {
            item.setCustomValidity("Telefone inválido");
        }
    }
}
...
function validateTELC(telc) {
    var number, digits, sum, i, result, equal_digits;
    equal_digits = 1;
    if (telc.length < 10)
        return false;
    for (i = 0; i < telc.length - 1; i++)
        if (telc.charAt(i) != telc.charAt(i + 1)) {
            equal_digits = 0;
            break;
        }
    if (!equal_digits) {
        number = telc.substring(0, 9);
        digits = telc.substring(9);
        sum = 0;
        for (i = 10; i > 1; i--)
            sum += number.charAt(10 - i) * i;
        result = sum % 11 < 2 ? 0 : 11 - sum % 11;
...
function maskTELC() {
    let strTELC = telc.value;
    if (strTELC.length == 3 || strTELC.length == 7) {
        telc.value += ".";
    } else if (strTELC.length == 11) {
        telc.value += "-";
    }
...
telc.addEventListener("input", function () { maskCPF() });
